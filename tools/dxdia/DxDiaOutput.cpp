...
void dxdia::WriteToFile(IDxcBlob *Blob, llvm::StringRef filename) {
  ...
  const bool fSuccess = WriteFile(hTempFile, contents.data(), contents.size(),
                                  &dwBytesWritten, NULL);
...
void dxdia::WritePDBToTmpFile(IDxcBlob *pdb, std::string *pdb_filename) {
  pdb_filename->clear();
